...
async def _convert_and_upload_wsi(
    wsi_id: str,
    patient_id: str,
    object_name: str,
):
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Récupération du WSI depuis MinIO
            data = await asyncio.to_thread(
                minio.get_object_bytes,
                object_name,
            )

            if data is None:
                raise FileNotFoundError(
                    f"WSI object not found in MinIO: {object_name}"
                )

            with open(in_path, "wb") as f:
                f.write(data)

            # Conversion SVS / TIFF / etc. -> DZI
            cmd = [
                vips_path,
                "dzsave",
                in_path,
            ...
            if result.returncode != 0:
                raise RuntimeError(
                    result.stderr
                    or f"vips failed with code {result.returncode}"
                )
            ...
            # Upload du fichier slide.dzi
            dzi_object = (
                f"patients/{patient_id}/dzi/{wsi_id}/slide.dzi"
            )
            ...
            print(
                "=== DZI Conversion Completed Successfully ==="
            )
            print(
                f"Uploaded {len(tile_files)} tiles "
                f"for WSI {wsi_id}"
            )
...
@router.post("/upload-and-convert", status_code=202)
async def upload_and_convert_wsi(
    background_tasks: BackgroundTasks,
    patient_id: str,
    ...
    # ----------------------------------------
    # 2. Génération du WSI ID
    # ----------------------------------------

    wsi_id = str(uuid.uuid4())

    object_name = (
        f"patients/{patient_id}/raw/wsi/"
        f"{wsi_id}/{file.filename}"
    )

    # ----------------------------------------
    # 3. Upload du fichier brut vers MinIO
    # ----------------------------------------
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"WSI upload failed: {str(e)}",
        )

    # ----------------------------------------
    # 4. Vérification de l'upload
    # ----------------------------------------
    ...
    if not exists:
        raise HTTPException(
            status_code=500,
            detail=(
                "WSI upload completed but "
                "object was not found in MinIO"
            ),
        )

    # ----------------------------------------
    # 5. Lancement automatique conversion DZI
    # ----------------------------------------
    ...
    return {
        "message": "WSI uploaded successfully and DZI conversion started",
        "patient_id": patient_id,
        "wsi_id": wsi_id,
        "object_name": object_name,
        "filename": file.filename,
