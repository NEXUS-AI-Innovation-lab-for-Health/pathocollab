...
export function MyPrerequisites({ onNavigate, onOpenPrerequisiteForm, onOpenScheduleModal, onOpenPrerequisitePreparation }: Props) {
  ...
  const openPathoCollabAnalysis = async (meeting: MyMeetingPrerequisites) => {
    ...
    try {
      ...
      // Fallback : le patient lié à la réunion vient des détails de la réunion (API)
      if (!patientId) {
        try {
          const meetingDetails = await fetchMeetingPrerequisiteDetails(meetingId, authService.getToken());
          if (meetingDetails.patient?.patient_id) {
            patientId = meetingDetails.patient.patient_id;
          }
        } catch (err) {
          console.warn('[PathoCollab] Impossible de récupérer le patient lié à la réunion :', err);
        }
      }
      ...
      const pathocollabPatientPayload = {
        id: patientId,
        patient_number: pathocollabPatientInfo?.patient_number ?? null,
        full_name: pathocollabPatientInfo
          ? `${pathocollabPatientInfo.firstname} ${pathocollabPatientInfo.lastname}`
          : `Patient ${patientId}`,
        date_of_birth: pathocollabPatientInfo?.dateofbirth ?? null,
        ...
        created_at: pathocollabPatientInfo?.created_at ?? null,
      };
      ...
      if (existingCaseId) {
        // Cas déjà créé : on met à jour les infos patient (au cas où elles auraient été
        // enregistrées avec des valeurs manquantes lors de la création initiale).
        if (pathocollabPatientInfo) {
          try {
            await fetch(`http://localhost:18002/api/patients/${encodeURIComponent(patientId)}`, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(pathocollabPatientPayload),
            });
          } catch (err) {
            console.warn('[PathoCollab] Impossible de mettre à jour les infos patient :', err);
          }
        }
      ...
      const response = await fetch('http://localhost:18002/api/cases/external', {
        ...
        body: JSON.stringify({
          ...
          patient: pathocollabPatientPayload,
